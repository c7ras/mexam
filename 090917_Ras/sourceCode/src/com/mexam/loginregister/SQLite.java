...
public class SQLite {
    ...
    static File temp = new File("account.sqlite");
    static String url = "jdbc:sqlite:" + temp.getAbsolutePath().replace("\\", "\\\\"); 
    //static String url = "jdbc:sqlite:C:\\Users\\RAS\\Documents\\NetBeansProjects\\090917_Ras\\src\\com\\mexam\\loginregister\\account.sqlite";
    static String error = "";
    ...
    public static boolean update(String table, String set, String id){
        ...
        try{
            SQLite.stmt = conn.createStatement();
            String query = "UPDATE "+ table +" SET " + set + " WHERE name='" + id +"'";
            stmt.executeUpdate(query);
            //You can include exception handling here. (e.g. Duplicate Data, etc.)
            result = true;
        }
    ...
    public static String[][] read(String table, String where){
        ...
        try{
            ...
            //Count total columns
            rs = stmt.executeQuery("SELECT DISTINCT * FROM " + table + " WHERE " + where);
            ResultSetMetaData rsmd = rs.getMetaData();
            int totalColumns = rsmd.getColumnCount();
    ...
    //Modify record
    public static boolean update(String table, byte[] image, String column, String id){
//      byte[] buffer = null;
        boolean result = false;
        try{
            String query = "UPDATE " + table +" SET "+ column +"=? WHERE name=" + id; 
            ResultSet rs = null;
            java.sql.PreparedStatement pstmt = null;            
